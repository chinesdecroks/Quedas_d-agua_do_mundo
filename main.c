    ...
    do {
        ...
        switch (opcao) {
            ...
            case 13: {
                ...
                Municipio* menor = municipio_com_menos_cachoeiras(lista_municipios);
                if (menor != NULL) {
                    printf("Municipio encontrado: %s (ID: %d)\n", menor->nome, menor->id_municipio);
                    printf("💧Quantidade de cachoeiras: %d\n", contar_cachoeiras_municipio(lista_municipios, menor->id_municipio));
                } else {
                    printf("⚠️ Nenhum municipio cadastrado na lista.\n");
                }
                break;
            }
